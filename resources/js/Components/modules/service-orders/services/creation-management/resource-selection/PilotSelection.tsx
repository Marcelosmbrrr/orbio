...
export const PilotSelection = React.memo((props: IProps) => {

    const [selections, setSelections] = React.useState<IItem[]>(props.getValues("pilot"));
    ...
    async function fetchData() {
        try {
            const ids = props.getValues('pilot').map((item: IItem) => item.id).toString();
            const args = "page=" + page + "&limit=" + limit + "&selections=" + ids;
            const response = await api.get(`api/v1/actions/service-orders/pilots?${args}`);
            setError(false);
            setList(response.data.pilots);
    ...
    function onSelectItem(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        const selections = props.getValues("pilot");
        ...
        if (isItemSelected(item_id)) {
            ...
            props.setValue("pilot", clone);
            setSelections(clone);
            return;
        }

        // Push selected record if not exists
        const record: IItem = list.filter((item: IItem) => Number(item.id) === Number(item_id))[0];
        const record_data = {
            id: record.id,
        ...
        props.setValue("pilot", [...selections, record_data]);
        ...
        <div className="h-full relative pt-5">
            <div className="flex flex-col gap-2">
                ...
                <div className="w-full">
                    <h3 className="text-lg font-bold text-gray-600 dark:text-white mb-1">Piloto (opcional)</h3>
                    <span>Lista de pilotos disponíveis para a ordem de serviço. Selecione uma das opções e clique em "Avançar Etapa".</span>
                </div>
                ...
                <div className="w-full flex">
                    <input value={search} onKeyDown={onSearch} onChange={(e) => setSearch(e.target.value)} type="search" placeholder="Procurar piloto" className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 border-gray-300 rounded-r dark:bg-gray-700 dark:border-l-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                </div>
