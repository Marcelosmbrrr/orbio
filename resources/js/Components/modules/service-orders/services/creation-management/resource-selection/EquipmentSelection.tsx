...
export const EquipmentSelection = React.memo((props: IProps) => {

    const [selections, setSelections] = React.useState<IItem[]>(props.getValues("equipments"));
    ...
    async function fetchData() {
        try {
            const ids = props.getValues('equipments').map((item: IItem) => item.id).toString();
            const args = "page=" + page + "&limit=" + limit + "&selections=" + ids;
            const response = await api.get(`api/v1/actions/service-orders/equipments?${args}`);
            setError(false);
            setList(response.data.equipments);
    ...
    function onSelectItem(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        const selections = props.getValues("equipments");
        ...
        if (isItemSelected(item_id)) {
            ...
            props.setValue("equipments", clone);
            setSelections(clone);
            return;
        }
        ...
        props.setValue("equipments", [...selections, record_data]);
        ...
        <div className="h-full relative pt-5">
            <div className="flex flex-col gap-2">
                ...
                <div className="w-full">
                    <h3 className="text-lg font-bold text-gray-600 dark:text-white mb-1">Equipamentos (opcional)</h3>
                    <span>Lista de equipamentos disponíveis para a ordem de serviço. Selecione uma ou mais opções e clique em "Avançar Etapa".</span>
                </div>

                <div className='flex'>
                    <CreateEquipment can_open={true} setReload={setReload} />
                ...
                <div className="w-full flex">
                    <button onClick={() => setReload(!reload)} type="button" className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-l text-gray-600 dark:text-white bg-white border border-gray-200 hover:bg-gray-100 hover:text-green-700 focus:z-10 focus:ring-2 focus:ring-green-700 focus:text-green-700 dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:focus:ring-green-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                        </svg>
                    </button>
                    <input value={search} onKeyDown={onSearch} onChange={(e) => setSearch(e.target.value)} type="search" placeholder="Procurar equipamento" className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 border-gray-300 rounded-r dark:bg-gray-700 dark:border-l-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                </div>
