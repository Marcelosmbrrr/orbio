...
export const FlightPlanSelection = React.memo((props: IProps) => {

    const [selections, setSelections] = React.useState<IItem[]>(props.getValues("flight_plans"));
    ...
    async function fetchData() {
        try {
            const ids = props.getValues('flight_plans').map((item: IItem) => item.id).toString();
            const args = "page=" + page + "&limit=" + limit + "&selections=" + ids;
            const response = await api.get(`api/v1/actions/service-orders/flight-plans?${args}`);
            setError(false);
            setList(response.data.flight_plans);
    ...
    function onSelectItem(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        const selections = props.getValues("flight_plans");
        ...
        if (isItemSelected(item_id)) {
            ...
            props.setValue("flight_plans", clone);
            setSelections(clone);
            return;
        }
        ...
        props.setValue("flight_plans", [...selections, record_data]);
        ...
        <div className="h-full relative pt-5">
            <div className="flex flex-col gap-2">
                ...
                <div className="w-full">
                    <h3 className="text-lg font-bold text-gray-600 dark:text-white mb-1">Planos de Voo (obrigatório)</h3>
                    <span>Lista de planos de voo disponíveis para a ordem de serviço. Selecione uma ou mais opções e clique em "Avançar Etapa".</span>
                </div>

                <div className='flex'>
                    <CreateFlightPlan can_open={true} />
                ...
                <div className="w-full flex">
                    <input value={search} onKeyDown={onSearch} onChange={(e) => setSearch(e.target.value)} type="search" placeholder="Procurar plano de vo" className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 border-gray-300 rounded-r dark:bg-gray-700 dark:border-l-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                </div>
