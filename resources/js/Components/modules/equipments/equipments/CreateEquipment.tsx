...
import { CreateEquipmentSchema } from './schemas/CreateEquipmentSchema';
...
type FormData = yup.InferType<typeof CreateEquipmentSchema>;

export const CreateEquipment = React.memo((props: IProps) => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue, reset } = useForm<FormData>({
        resolver: yupResolver(CreateEquipmentSchema)
    });

    async function onSubmit(form: FormData) {
        try {
            setPending(true);
            await api.post("api/v1/modules/equipments", form, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            enqueueSnackbar("Equipamento criado com sucesso!", { variant: "success" });
            setTimeout(() => {
                setOpen(false);
