...
import { CreateDroneSchema } from './schemas/CreateDroneSchema';
...
type FormData = yup.InferType<typeof CreateDroneSchema>;

export const CreateDrone = React.memo((props: IProps) => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue, reset } = useForm<FormData>({
        resolver: yupResolver(CreateDroneSchema)
    });

    async function onSubmit(form: FormData) {
        try {
            setPending(true);
            await api.post("api/v1/modules/drones", form, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                },
                responseType: 'json'
            })
            enqueueSnackbar("Drone criado com sucesso!", { variant: "success" });
            setTimeout(() => {
                setOpen(false);
