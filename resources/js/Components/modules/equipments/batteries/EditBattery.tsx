...
import { EditBatterySchema } from './schemas/EditBatterySchema';
...
type FormData = yup.InferType<typeof EditBatterySchema>;

export const EditBattery = React.memo((props: IProps) => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue, reset } = useForm<FormData>({
        resolver: yupResolver(EditBatterySchema)
    });
    ...
    function setDefaultValues() {
        ...
        setValue('last_charge', props.selection.last_charge, { shouldValidate: true });
    ...
    async function onSubmit(form: FormData) {
        ...
        _form.append("last_charge", String(form.last_charge));  // @ts-ignore
        _form.append("image", form.image);
        _form.append('_method', 'PATCH');

        try {
            setPending(true);
            await api.post("api/v1/modules/batteries/" + props.selection.id, _form, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                },
                responseType: 'json'
            });
            enqueueSnackbar("Bateria atualizada com sucesso!", { variant: "success" });
            setTimeout(() => {
                setOpen(false);
                                ...
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    <div className="grid gap-4 mb-4 sm:grid-cols-1">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" {...register('serial_number')} id="serial" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Informe o número serial da bateria" />
                                            <span className='text-red-500 text-sm'>{errors.serial_number?.message}</span>
                                        </div>
                                        <div>
                                            <label htmlFor="last_charge" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Carga</label>
                                            <input type="date" {...register('last_charge')} id="last_charge" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-auto p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></input>
                                            <span className='text-red-500 text-sm'>{errors.last_charge?.message}</span>
                                        </div>
                                        <div>
