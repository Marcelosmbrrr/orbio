...
import { CreateBatterySchema } from './schemas/CreateBatterySchema';
...
type FormData = yup.InferType<typeof CreateBatterySchema>;

export const CreateBattery = React.memo((props: IProps) => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue, reset } = useForm<FormData>({
        resolver: yupResolver(CreateBatterySchema)
    });

    async function onSubmit(form: FormData) {
        try {
            setPending(true);
            await api.post("api/v1/modules/batteries", form, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
            enqueueSnackbar("Bateria criada com sucesso!", { variant: "success" });
            setTimeout(() => {
                setOpen(false);
