...
import { EditFlightPlanSchema } from './schemas/EditFlightPlanSchema';
...
type FormData = yup.InferType<typeof EditFlightPlanSchema>;

export const EditFlightPlan = React.memo((props: IProps) => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue } = useForm<FormData>({
        resolver: yupResolver(EditFlightPlanSchema)
    });
    ...
    async function onSubmit(form: FormData) {
        try {
            const response = await api.patch("api/v1/modules/flight-plans/" + props.selection.id, form);
            enqueueSnackbar("Plano de voo atualizado com sucesso!", { variant: "success" });
            setTimeout(() => {
                setOpen(false);
                                    ...
                                    <form onSubmit={handleSubmit(onSubmit)}>
                                        <div className="grid gap-4 mb-4 sm:grid-cols-1">
                                            <div>
                                                ...
                                                <input type="text" {...register("name")} id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Informe o nome do plano" />
                                            </div>
                                        </div>
                                        <div className="flex justify-end gap-1">
                                            <button onClick={onClose} type="submit" className="py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                                Cancelar
                                            </button>
                                            <button type="submit" className="focus:outline-none text-white bg-green-800 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">
