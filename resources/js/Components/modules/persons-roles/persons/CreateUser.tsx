...
export const CreateUser = React.memo((props: IProps) => {
    ...
    async function onSubmit(form: FormData) {
        try {
            setPending(true);
            await api.post("api/v1/modules/persons-roles/users", form);
        ...
        if (user?.role.id == "1") {
            return (
                <div>
                    ...
                    <input disabled value={"Gerente"} id="role" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                </div>
            )
        } else {
            return <SelectFetchedData id={"role_id"} label={"Cargo"} default={"none"} fetchFrom={"api/v1/actions/roles?select=id,name"} setValue={setValue} />
        }
    }
