...
export const DeleteModuleRecord = React.memo((props: IProps) => {

    const [open, setOpen] = React.useState(false);
    const [pending, setPending] = React.useState(false);
    const { enqueueSnackbar } = useSnackbar();

    const message = props.selections.length === 1 ? `Deleção ${props.type === "soft" ? "reversível" : "permanente"} do registro selecionado.` : `Deleção ${props.type === "soft" ? "reversível" : "permanente"} dos ${props.selections.length} registros selecionados.`;

    async function onSubmit() {
        try {
            setPending(true);
            const response = await api.delete(props.route, { data: { ids: props.selections } });
